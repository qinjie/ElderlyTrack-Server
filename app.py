    ...
    with conn.cursor() as cur:
        ...
        for row in cur:
            ...
            with conn.cursor() as cur2:
                #Get beacon list
                sql = "SELECT id, resident_id, uuid, major, minor, status, created_at FROM beacon WHERE resident_id={}".format(row[0])
                cur2.execute(sql)
                for row2 in cur2:
                    beacons.append({"id":row2[0],"resident_id":row2[1],"uuid":row2[2],"major":row2[3],"minor":row2[4],"status":row2[5],"created_at":str(row2[6])})
                with conn.cursor() as cur3:
                    sql = "SELECT id,beacon_id,locator_id,user_id,longitude,latitude,address,created_at FROM location_history WHERE created_at>=ADDDATE(CURRENT_TIMESTAMP,INTERVAL -30 DAY) AND beacon_id={} ORDER BY created_at DESC LIMIT 5".format(row2[0])
                    cur3.execute(sql)
                    for row3 in cur3:
                        locations.append({"id":row3[0],"beacon_id":row3[1],"locator_id":row3[2],"user_id":row3[3],"longitude":str(row3[4]),"latitude":str(row3[5]),"address":row3[6],"created_at":str(row3[7])})
...
def index():
    ...
    for row in cur:
        ...
        #Get the beacon list
        with conn.cursor() as cur2:
            cur2.execute("SELECT id,resident_id,uuid,major,minor,status,created_at FROM beacon WHERE resident_id={}".format(row[0]))
        for row2 in cur2:
            beacons.append({"id":row2[0],"resident_id":row2[1],"uuid":row2[2],"major":row2[3],"minor":row2[4],"status":row2[5],"created_at":str(row2[6])})
            beacon_id.append(row2[0])
        #Get relatives list
        with conn.cursor() as cur2:
            cur2.execute("SELECT caregiver.relative_id, user.username, user.email, user.access_token, user.email_confirm_token, user.role, user.phone_number, user.status, user.allowance, user.timestamp FROM caregiver INNER JOIN user ON caregiver.relative_id=user.id WHERE caregiver.resident_id={} ORDER BY caregiver.relative_id ASC".format(row[0]))
            for row2 in cur2:
                relatives.append({"id":row2[0],"username":row2[1],"email":row2[2],"access_token":row2[3],"email_confirm_token":row2[4],"role":row2[5],"phone_number":row2[6],"status":row2[7],"allowance":row2[8],"timestamp":str(row2[9])})  
        #Get locations list
        with conn.cursor() as cur2:
            for beacon in beacon_id:
                response = cur2.execute("SELECT id,beacon_id,locator_id,user_id,longitude,latitude,address,created_at FROM location_history WHERE created_at >= ADDDATE(CURRENT_TIMESTAMP, INTERVAL -30 DAY) AND beacon_id={} ORDER BY created_at DESC LIMIT 5".format(beacon))
                for row2 in cur2:
                    locations.append({"id":row2[0],"beacon_id":row2[1],"locator_id":row2[2],"user_id":row2[3],"longitude":str(row2[4]),"latitude":str(row2[5]),"address":row2[6],"created_at":str(row2[7])})
        result.append({"id":row[0],"fullname":row[1],"dob":str(row[2]),"nric":row[3],"status":row[4],"created_at":str(row[5]),"reported_at":str(row[6]),"remark":row[7],"hide_photo":row[8],"image_path":row[9],"thumbnail_path":row[10],"beacons":beacons,"relatives":relatives,"locations":locations})
...
def report_resident():
    ...
    status=0
    with conn.cursor() as cur:
        sql_response = cur.execute("SELECT status FROM missing WHERE resident_id={} AND reported_by={} AND created_at>=CURRENT_DATE".format(resident_id, reported_by))
        if sql_response == 0:
            with conn.cursor() as cur:
                sql = "INSERT INTO missing (resident_id,reported_by,remark) VALUES ({},{},'{}')".format(resident_id,reported_by,remark)
                cur.execute(sql)
            with conn.cursor() as cur:
            ...
            for row in cur:
                if row[0]==0:
                    ...
                    status=0
            with conn.cursor() as cur:
                sql = "UPDATE missing SET status={},remark='{}' WHERE resident_id={} AND reported_by={} AND created_at>=CURRENT_DATE".format(status, remark, resident_id, reported_by)
                cur.execute(sql)   
            with conn.cursor() as cur:
...
def report_found_resident():
    ...
    with conn.cursor() as cur:
        ...
        if sql_response == 0:
            with conn.cursor() as cur:
                sql = "INSERT INTO location (beacon_id,user_id,longitude,latitude) VALUES ({},{},{},{})".format(beacon_id,user_id,longitude,latitude)
                cur.execute(sql)
            message = "Successfully insert"
        else:
            with conn.cursor() as cur:
                sql = "UPDATE location SET longitude={},latitude={},user_id={} WHERE beacon_id={}".format(longitude,latitude,user_id,beacon_id)
                cur.execute(sql)
            message = "Successfully updated"
    with conn.cursor() as cur:
        sql = "INSERT INTO location_history (beacon_id,user_id,longitude,latitude) VALUES ({},{},{},{})".format(beacon_id,user_id,longitude,latitude)
        cur.execute(sql)
    conn.commit()
    conn.close()
...
def prepare_message(beacon_id):
    ...
    with conn.cursor() as cur:
        sql = "SELECT DISTINCT user.endpointID FROM user INNER JOIN caregiver on user.id=caregiver.relative_id AND caregiver.resident_id={}".format(resident_id)
        cur.execute(sql)
